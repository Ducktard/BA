...
Meteor.methods({
      ...
      moveToNextLevel(userId,maxLevel){
            ...
            console.log("maxLevel: " + maxLevel + " currentLevel: " + currentLevel);
            if(maxLevel > currentLevel){
              nextLevel = currentLevel+1;
              console.log("nach if :   nextLevel: " + nextLevel);
              Meteor.users.update({"_id":userId}, {
                $set:
                  {"profile.level": nextLevel,
                  "profile.reachedLevel": new Date()}
                });
              console.log('Nächstes Level: ' + Meteor.user().profile.level);
            }else{
              console.log('Es existieren leider keine neuen Level');
            }
